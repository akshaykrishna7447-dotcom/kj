...
const MARQUEE = [
    'ആധികാരിക പൈതൃകം', '✦', 'വൈദിക പാരമ്പര്യം', '✦',
    'പവിത്ര കർമ്മങ്ങൾ', '✦', 'ദിവ്യ അനുഗ്രഹം', '✦',
    'കേരള വാസ്തുവിദ്യ', '✦', 'ആത്മീയ സങ്കേതം', '✦',
    'ആധികാരിക പൈതൃകം', '✦', 'വൈദിക പാരമ്പര്യം', '✦',
];
...
export default function Hero({ isLoaded }) {
    ...
    const letters = ['കൊ', 'റ്റി', 'വ', 'ട്ട', 'ത്ത്'];
    ...
    return (
        <section id="hero" ref={heroRef} style={{ height: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {/* Hero content */}
            <div ref={contentRef} style={{ position: 'relative', zIndex: 10, textAlign: 'center', maxWidth: '1200px', width: '100%', padding: '0 32px', transform: 'translateY(40px)' }}>

                <h1 style={{ display: 'flex', flexDirection: 'column', gap: '4px', overflow: 'visible' }}>
                    <span style={{ display: 'block', textAlign: 'center', overflow: 'visible', paddingBottom: '0.08em' }}>
                        {letters.map((char, i) => (
                    ...
                    <span ref={subtitleRef} style={{
                        fontFamily: "'Noto Serif Malayalam',serif", fontWeight: 500,
